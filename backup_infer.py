...
def main(_):
    #newly added
    AKEY = 'aio_eGgc37UkOxHGA9mv1JPdjyPGY7Hx'
    AUSER = 'MCQxCLH'
    aio = Client(AUSER,AKEY)
    num = 0
    prevNum = 999
    rightState = 0
    leftState = 0
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            if shape is None:
                ...
                adaData = aio.data('icon-commands')
                num = len(adaData)

                if num > prevNum:
                    command = adaData[0].value
...
##                print('')
                ...
                #若按下 q 鍵則離開迴圈, waitKey(0),0 = 0ms, 则表示程序会无限制的等待用户的按键事件
